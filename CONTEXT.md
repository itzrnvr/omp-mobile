...
- App→TUI steering WORKS via the extension queue + agent_end-hook delivery (verified 2026-09-05: steered message appeared in the TUI turn at the boundary). Facts: immediate pi.sendUserMessage is a silent no-op (does not throw); sendUserMessage is absent from Object.keys(pi) top level and from pi.extension/pi.runtime sub-objects but the call resolves (prototype); event-hook ctx exposes only {ui}. ext_steer transport works after last-hello-wins eviction. LONG-RUNNING omp PROCESSES RUN STALE EXTENSION CODE - restart the TUI after editing the extension. Second-writer safety: owned sessions never spawn; mtime-fresh guard covers the ext-down window.
...
- Steering delivery semantics FINAL (2026-09-05, both modes verified single-delivery via raw-JSONL user-entry counts): MID-TURN sends inject immediately via sendUserMessage (ext tracks tuiRunning via agent_start/agent_end) and clear the queue on no-throw (ack mode=mid); IDLE sends queue and deliver at the next turn boundary via the agent_end hook (ack mode=idle, chip says queued-for-next-turn and clears on that boundary agent_start). Proto in EVERY ext_hello (onopen + sid re-hello) - omitting it on re-hello made the owner look stale and blocked routing. Pre-created-session-file story UNOBSERVED and moot (resume-resolve reverted; event-driven binding only).
...
Tooling hazard (2026-09-08/09): Bun caches compiled extensions across omp restarts — editing the file is NOT enough; rename the directory (omp-mobile-sync -> v3) + update enabled.txt to bust it. D: hitting 0 bytes zeroed mobile/src/store/index.ts mid-write (reconstructed from git + re-applied hunks); commit before big builds and keep GBs of headroom (gradle + proxy logs eat it fast). uiautomator dumps go stale/empty under rapid list updates — a missing node proves nothing; scroll + screenshot decides.

Scroll pin (2026-09-09): the pin releases ONLY on user drag (onScrollBeginDrag) and re-engages at drag/momentum end near the bottom; per-event distance gating permanently disengaged it on VirtualizedList estimate re-measures. ListFooterComponent was never broken — "missing Working header" sightings were all above-the-fold viewport on giant single-cell content (proven by bottom-scroll showing FOOTER_DBG + Working timer).
